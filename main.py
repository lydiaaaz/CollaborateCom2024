...
from model.JSTH import MainModel
...
parser.add_argument('--prefix', type=str, default='std', help='prefix to name a trial')
...
parser.add_argument('--predictor', type=str, default="fusion", choices=["linear", "merge", "fusion"], help="type of predictor")
...
parser.add_argument('--use_static', action='store_false', help='whether use static embedding for users')
parser.add_argument('--use_dynamic', action='store_false', help='whether use dynamic embedding for users and cascades')
parser.add_argument('--use_structural', action='store_false',
                    help='whether to adopt structural learning in the cascade embedding module')
parser.add_argument('--use_temporal', action='store_false',
...
parser.add_argument('--hawkes', type=float, default=5e-5,
                    help='the weight to add hawkes loss')
try:
...
for num in range(param['run']):
    ...
    model = MainModel(device=device, node_dim=param['node_dim'], embedding_module_type=param['embedding_module'],
    ...
    # metric = Metric(path=f"{param['result_path']}_{num}.txt", logger=logger, fig_path=f"fig/{param['prefix']}")
    ...
    # model.load_state_dict(torch.load('saved_models/std_weibo_0.pth'))
