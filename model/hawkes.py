...
def compute_integral_unbiased(model, data, time):
    ...
    temp_hid = torch.sum(temp_hid, dim=1, keepdim=True)
...
class Hawkes(nn.Module):
    ...
    def forward(self, data):
        """
        计算 Hawkes 过程的强度函数
        :param data: [batch_size, length, d_model]，隐藏状态序列
        :return: 强度函数
        """
        # 将隐藏状态通过线性变换转换成标量
        intensity = self.linear(data)
        # 使用 softplus 函数将标量转换成非负数，表示事件的强度
        intensity = softplus(intensity, self.beta)
        return intensity
