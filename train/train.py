...
def eval_model(model: MainModel, eval: Data, device: torch.device, param: Dict, metric: Metric,
    ...
    with torch.no_grad():
        for x, label in tqdm(eval.loader(param['bs']), total=math.ceil(eval.length / param['bs']), desc='eval_or_test'):
            ...
            pred, event_ll, non_event_ll = model.forward(src, dst, trans_cas, trans_time, pub_time, target_idx)
...
def train_model(num: int, dataset: Data, model: MainModel, logger: logging.Logger, early_stopper: EarlyStopMonitor,
    ...
    for epoch in range(param['epoch']):
        ...
        for x, label in tqdm(train.loader(param['bs']), total=math.ceil(train.length / param['bs']),
            ...
            trans_time, pub_time, label = move_to_device(device, trans_time, pub_time, label)
            pred, event_ll, non_event_ll = model.forward(src, dst, trans_cas, trans_time, pub_time, target_idx)
            hawkes_loss = -torch.sum(event_ll - non_event_ll)
            if sum(target_idx) > 0:
                ...
                optimizer.zero_grad()
                original_loss = loss_criterion(target_pred, target_label)
                loss = original_loss + hawkes_loss # * param['hawkes']
                # print("original loss:", original_loss)
                # print("hawkes loss loss:", hawkes_loss)
                loss.backward()
                optimizer.step()
