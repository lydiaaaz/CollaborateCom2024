...
class MessageGenerator(nn.Module):
    ...
    # 接收一批交互的信息，根据给定的目标类型为每个交互生成消息，然后通过聚合节点的所有消息(一个节点可能出现在多个交互中，因此它可能有多个消息)为每个节点生成唯一消息
...
def set_message_function(single: bool, raw_message_dim: Dict[str, int], message_dim: int) -> \
    ...
    if single:  # 用户的消息生成函数为级联消息生成函数
        user_src_mf = cas_mf
        user_dst_mf = cas_mf
    else:  # 默认，用户的消息生成函数为独立的消息生成函数
        user_src_mf = get_message_function('mlp', raw_message_dim['user'], message_dim)
        user_dst_mf = get_message_function('mlp', raw_message_dim['user'], message_dim)  # 对源用户和目标用户的表征更新处理相同
